...
def parse_command_line():
    ...
    parser.add_argument('-k', '--api_key', metavar = 'API_KEY', help = 'NASA developer key')
...
def create_date(datelist, surprise):
    ...
    #TODO: Your code goes here
    lowdate = date(1995, 6, 16)
    highdate = date.today()
    oneday = timedelta(days = 1)
    try:
        if datelist == None and surprise == True:
            delta = highdate - lowdate
            rand = randint(0, delta.days)
            offset = timedelta(days = rand)
            return lowdate + offset
        if datelist == None and surprise == False:
            return highdate - oneday
        month, day, year = datelist
        d = date(year, month, day)
        if d < lowdate or d > highdate:
            return None
        return d
    except:
        return None
...
def query_url(d, api_key):
    ...
    full_url = base_url + "api_key=" + api_key + "&date=" + format_date
    return full_url
...
def save_image(d, image):
    ...
    fullpath = year_month + "/" + year + "-" + month + "-" + day + ".jpg"
